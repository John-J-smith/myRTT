...
uint8_t GPIOA_Status_Match(GPIOA_TypeDef *GPIOx, GPIO_InitType *InitStruct)
{
    uint32_t tmp_reg1, tmp_reg2, tmp_reg3;
    ...
    tmp_reg2 &= InitStruct->GPIO_Pin;
    tmp_reg3 = GPIOx->ATT;
    tmp_reg3 &= InitStruct->GPIO_Pin;    

    if(GPIO_Mode_OUTPUT_CMOS == InitStruct->GPIO_Mode)
    {
        if((0 == tmp_reg1) && (0 == tmp_reg2) && (0 == tmp_reg3))
        {
            return 1;
        }         
    }
    else if(GPIO_Mode_INPUT == InitStruct->GPIO_Mode)
    {
        if((1 == tmp_reg1) && (1 == tmp_reg2))
        {
            return 1;
        }      
    }
...
uint8_t GPIOBToF_Status_Match(GPIO_TypeDef *GPIOx, GPIO_InitType *InitStruct)
{
    uint32_t tmp_reg1, tmp_reg2, tmp_reg3;
