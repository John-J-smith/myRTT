...
{
    size_t i;
    uint32_t addr = ht6xxx_onchip_flash.addr + offset;
...
static int erase(long offset, size_t size)
{
    ...
    {
        HT_Flash_PageErase(addr + (PAGE_SIZE * i));
    }
    FLASH_Lock();
...
const struct fal_flash_dev ht6xxx_onchip_flash =
{
    .name       = "flash_onchip",
    .addr       = 0x00000000,
    .len        = 512*1024,
